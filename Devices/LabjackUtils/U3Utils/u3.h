...
struct U3_CALIBRATION_INFORMATION {
  ...
  double ainSESlope;      //lvAINSESlope
  double ainSEOffset;     //lvAINSEOffset
  double ainDiffSlope;    //lvAINDiffSlope
  double ainDiffOffset;   //lvAINDiffOffset
  double hvAINSlope[4];
  double hvAINOffset[4];
  double dacSlope[2];
  double dacOffset[2];
  double tempSlope;
  double tempSlopeLow;
...
struct U3_LJTDAC_CALIBRATION_INFORMATION{
  uint8 prodID;
  double DACSlopeA;
  double DACOffsetA;
  double DACSlopeB;
...
typedef struct U3_LJTDAC_CALIBRATION_INFORMATION u3LJTDACCalibrationInfo;
...
long getLJTDACCalibrationInfo( HANDLE hDevice,
                               u3LJTDACCalibrationInfo *caliInfo,
                               uint8 DIOAPinNum);
...
long binaryToCalibratedAnalogVoltage( u3CalibrationInfo *caliInfo,
                                      int dac1Enabled,
                                      uint8 negChannel,
                                      uint16 bytesVoltage,
...
//bytesVoltage = the 2 byte voltage that needs to be converted
//analogVoltage = the converted analog voltage

long binaryToCalibratedAnalogVoltage_hw130( u3CalibrationInfo *caliInfo,
...
long analogToCalibratedBinaryVoltage( u3CalibrationInfo *caliInfo,
                                      int DACNumber,
                                      double analogVoltage,
                                      uint8 *bytesVoltage);
...
//analogVoltage = the analog voltage that need to be converted
//bytesVoltage = the converted binary 8 bit value

long analogToCalibratedBinary8BitVoltage( u3CalibrationInfo *caliInfo,
                                          int DACNumber,
                                          double analogVoltage,
                                          uint8 *bytesVoltage8);
//Translates a analog output voltage value (Volts) to a binary 8 bit value
...
//DACNumber - channel number of the DAC
//analogVoltage = the analog voltage that need to be converted
//bytesVoltage8 = the converted binary 8 bit value

long analogToCalibratedBinary16BitVoltage( u3CalibrationInfo *caliInfo,
                                           int DACNumber,
                                           double analogVoltage,
                                           uint16 *bytesVoltage16);
...
//caliInfo = structure where calibrarion information is stored
//DACNumber - channel number of the DAC
//analogVoltage = the analog voltage that needs to be converted
//bytesVoltage16 = the converted binary 16 bit value

long LJTDACAnalogToCalibratedBinaryVoltage( u3LJTDACCalibrationInfo *caliInfo,
                                            int DACNumber,
                                            double analogVoltage,
                                            uint16 *bytesVoltage);
...
long binaryToCalibratedAnalogTemperature( u3CalibrationInfo *caliInfo,
...
//Returns -1 on error, 0 on success.
//caliInfo = structure where calibrarion information is stored
//bytesTemperature = the 2 byte binary temperature that needs to be converted
//analogTemperature = the converted Kelvins temperature

long binaryToUncalibratedAnalogVoltage( int dac1Enabled,
                                        uint8 negChannel,
                                        uint16 bytesVoltage,
                                        double *analogVoltage);
//Translates the binary analog input bytes read from the U3, to a voltage value
...
long binaryToUncalibratedAnalogVoltage_hw130( int highVoltage,
                                              uint8 positiveChannel,
                                              uint8 negChannel,
                                              uint16 bytesVoltage,
                                              double *analogVoltage);
...
long analogToUncalibratedBinary8BitVoltage( double analogVoltage,
                                            uint8 *bytesVoltage8);
//Translates a DAC voltage value (Volts) to a binary 8 bit value (uncalibrated)
//that can be sent to a U3.  Returns -1 on error, 0 on success.
//This function is for U3 hardware versions 1.20 and 1.21.
...
long analogToUncalibratedBinary16BitVoltage( double analogVoltage,
                                             uint16 *bytesVoltage16);
