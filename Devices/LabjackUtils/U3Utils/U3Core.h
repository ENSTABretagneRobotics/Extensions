...
#define TEMPERATURE_SENSOR_CHANNEL_U3 30

// Negative AIN channel for single-ended channels.
#define NEGATIVE_CHANNEL_FOR_SINGLE_ENDED_U3 31

// If the device has a hardware version number less than 1.21, define 
// OLD_HARDWARE_VERSION_U3. The version of the driver installed must also support
...
inline long ConfigWatchdogU3(long hDevice, BOOL bEnable, SHORT timeout)
{
	if (bEnable)
	{
...
inline long OpenU3USB(char* address, HANDLE* phDevice, u3CalibrationInfo* pCaliInfo)
{
...
inline long ResetU3USB(HANDLE hDevice, BOOL bHardReset)
{
  ...
  recChars = LJUSB_BulkRead(hDevice, U3_PIPE_EP1_IN, recBuff, 4);
  if (recChars < 4)
  {
    if (recChars == 0)
...
inline long ConfigWatchdogU3(HANDLE hDevice, BOOL bEnable, SHORT timeout)
{
...
inline long CloseU3USB(HANDLE hDevice)
{
